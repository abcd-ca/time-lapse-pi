...
pub fn parse_duration_to_ms(duration: &str) -> Result<u64, &'static str> {
    if duration.len() == 2 {
        ...
        if num == 0 {
            Err("Duration must be at least 1h")
        } else {
            ...
            _ => Err("Unexpected time unit type. Use h for hours, d for days or w for weeks. Ex. 3h, 2d, 1w")
        }.unwrap();
            Ok(duration_ms)
        }
    } else {
        Err("Expected duration to be two characters, ##u where n is a number and u is the unit of time like hours, minutes or days, ex. 6h, 30m, 20d")
    }
}
